  ...
  return (
    <span className="relative inline-flex">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="grid h-[18px] w-[18px] cursor-help place-items-center rounded-full border border-line-strong text-[10px] font-bold text-ink-2 transition hover:border-accent hover:text-accent focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40"
      >
        i
      </button>
      {open && (
        <span
          id={id}
          role="tooltip"
          className={`rise-in pointer-events-none absolute right-0 z-50 ${width} rounded-lg border border-line bg-surface p-3 text-left text-[11px] leading-relaxed font-normal normal-case tracking-normal text-ink-2 shadow-[var(--shadow)] ${
