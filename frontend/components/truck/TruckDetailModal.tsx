      ...
      <div className="grid grid-cols-1 gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
        {FIELD_GROUPS.map((group) => (
          <section key={group.id} className="rounded-lg border border-line bg-surface-2 p-3">
            <h3 className="text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-3">{group.title}</h3>
                ...
                return (
                  ...
                  >
                    <dt className="min-w-0 flex-1">
                      <span className="block truncate text-[11px] font-medium text-ink" title={info?.label ?? field}>
                        {info?.label ?? field}
                      </span>
                      <span className="num block truncate text-[9px] text-ink-3">{field}</span>
                    </dt>
