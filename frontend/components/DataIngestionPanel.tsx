...
/** What the host's `onProvisionSite` resolves to so the form can show the truth. */
...
export interface DigitalTwinDashboardProps {
  ...
  sites?: SiteConfig[];
  activeSiteId?: string;
  /** May be sync (legacy) or async; when async its outcome is surfaced verbatim. */
...
function SiteProvisioner({
  ...
  // Wiring only -- no class names below are altered.  The host decides what a
  // provision actually does (a real fetch in `twin-view.tsx`); we surface its
  // outcome verbatim and never let a rejected promise escape into React.
  const submit = async () => {
    ...
    if (!onProvisionSite) {
      setMessage("Host app has not wired onProvisionSite yet.");
      return;
    }
  ...
  return (
    <section className={`${CARD} px-5 py-5`}>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {sites.map((site) => (
          <span key={site.siteId} className={`rounded-full px-3 py-1 text-[11px] ${site.siteId === activeSite.siteId ? "bg-cyan-400/10 text-cyan-200" : "text-slate-500"}`}>
            {site.label}
          </span>
        ))}
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div>
          <label className={label} htmlFor="site-id">Site ID</label>
          <input id="site-id" className={field} value={draft.siteId} onChange={(e) => setDraft({ ...draft, siteId: e.target.value })} placeholder="SWP-PUNE-01" />
        </div>
        <div>
          <label className={label} htmlFor="site-customer">Customer</label>
          <input id="site-customer" className={field} value={draft.customer} onChange={(e) => setDraft({ ...draft, customer: e.target.value })} />
        </div>
        <div>
          <label className={label} htmlFor="site-chargers">Chargers</label>
          <input id="site-chargers" type="number" min={0} className={field} value={draft.chargers} onChange={(e) => setDraft({ ...draft, chargers: Number(e.target.value) })} />
        </div>
        <div>
          <label className={label} htmlFor="site-dg">DG Capacity (kW)</label>
          <input id="site-dg" type="number" min={0} className={field} value={draft.dgCapacityKw} onChange={(e) => setDraft({ ...draft, dgCapacityKw: Number(e.target.value) })} />
        </div>
        <div className="col-span-2">
          <label className={label} htmlFor="site-feeder">Grid Feeder (kW)</label>
          <input id="site-feeder" type="number" min={0} className={field} value={draft.gridFeederKw} onChange={(e) => setDraft({ ...draft, gridFeederKw: Number(e.target.value) })} />
        </div>
      </div>
      ...
      <button
        ...
        onClick={submit}
        disabled={submitting}
        className="mt-4 w-full rounded-lg bg-cyan-400/90 px-4 py-2.5 text-xs font-semibold text-slate-950 transition hover:bg-cyan-300 disabled:cursor-not-allowed disabled:opacity-60"
      >
      ...
      <p className="mt-3 text-[11px] leading-relaxed text-slate-600">
        The validated feed carries no site key, so this twin is fleet-wide. Once the data layer emits site attribution,
        pass <code className="font-mono text-slate-500">sites[].vehicleFilter</code> and every panel scopes with no further changes.
      </p>
    </section>
  );
}
